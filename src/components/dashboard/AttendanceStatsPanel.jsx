 ...
 * }} props
 */
export function AttendancePerformanceCard({ stats }) {
  const { attended, missed, totalDays, performanceRate } = stats;
 ...
 * }} props
 */
export function AttendanceProgressCard({ stats }) {
  const { attended, missed, remaining, totalDays, overallProgress } = stats;
  ...
  return (
    <div className="rounded-lg border border-border bg-surface p-5">
      ...
      <p className="mt-3 text-xs text-muted-foreground">Total conference days: {totalDays}</p>
    </div>
  );
}

/**
 * Side-by-side performance + progress (dashboard conference view).

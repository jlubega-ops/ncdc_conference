...
export async function POST(request) {
  ...
  try {
    ...
    const url = await savePublicUpload("organiser-logos", compressed, filename);
    await logActivity({
      session,
      request,
      action: ACTIVITY_ACTIONS.UPLOAD_ORGANISER_LOGO,
      description: "Uploaded organiser logo",
      resourceType: "upload",
      metadata: { url },
    });
    ...
    return NextResponse.json({
      url,
      message: "Organisation logo uploaded successfully.",
    });
  } catch (error) {
